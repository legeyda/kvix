...
class Action(BaseAction):
    def _run(self) -> None:
        ...
        def create_dialog(builder: DialogBuilder, key_prefix: str = '', conf: Conf = self.action_type.context.conf, title_prefix: str = ''):
            for key, scope in conf.scopes().items():
                create_dialog(builder, key_prefix + key + '.', scope, title_prefix + (scope.title + ' | ' if scope.title else ''))
            for key, item in conf.items().items():
                create_entry(builder, key_prefix + key + '.', item, title_prefix + (item._title or item.key))
...
class Plugin(BasePlugin):
    ...
    def _create_single_action_type(self) -> ActionType:
        return BaseActionType(self.context, 'kwix.plugin.builtin.settings', str(action_type_title_text), Action)
    def get_actions(self) -> list[Action]:
        return [Action(self._single_action_type, str(default_action_title_text), default_action_description )]