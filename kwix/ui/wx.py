...
class Ui(BaseUi):
	...
	def run(self):
		...
		# todo create fake hidden frame to avoid exiting mainloop on closing last frame
		self._app.MainLoop()
	...
	def destroy(self):
		self._exec_in_mainloop(self._app.Destroy)
...
class ModalWindow:
	...
	def _init_window(self):
		self._root = wx.Frame(None, wx.ID_ANY, "kwix")
		self._root.Size = wx.Size(500, 500)
	...
	def _show(self):
		self._parent._exec_in_mainloop(self._root.Show)
	def _hide(self) -> None:
		self._parent._exec_in_mainloop(self._root.Hide)
	def destroy(self):
		self._parent._exec_in_mainloop(self._root.Destroy)
...
class Selector(ModalWindow, BaseSelector):
	...
	def __init__(self, parent: Ui, item_source: ItemSource = EmptyItemSource()):
		...
		BaseSelector.__init__(self, item_source)
		self.result: Item | None = None
		self._init_window()

	def _init_window(self):
		ModalWindow._init_window(self)

		self._search_textbox = wx.TextCtrl(self._root)

		self._result_listbox = wx.ListBox(self._root, wx.LB_SINGLE)

		box_layout = wx.BoxSizer(wx.VERTICAL)
		box_layout.Add(self._search_textbox)
		box_layout.Add(self._result_listbox)

		self._root.AddChild(box_layout)
		...
		self._search_textbox = ttk.Entry(self._mainframe, textvariable=self._search_query)
		self._search_textbox.grid(column=0, row=0, sticky='ew')
		for key in ('<Up>', '<Down>', '<Control-Home>', '<Control-End>'):
			self._search_textbox.bind(key, self._on_search_entry_keys) # todo bind _window and route to widgets
	...
	def _do_show(self):
		super()._do_show()
		self._search_textbox.focus_set()
		self._on_query_entry_type()
		self._search_textbox.select_range(0, 'end')
